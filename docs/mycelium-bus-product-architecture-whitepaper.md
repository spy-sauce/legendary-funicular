...
1. **Failure / timeout paths.** What happens when a capability-biome is down, slow, or returns garbage? The Frank fleet already has fallback chains internally; the *bus* needs its own story — error nutrients, retry policy, fallback-biome selection, and how partial failures surface to the user. (Touched in runtime step 6; needs full design.)
...
3. **Async / long-running replies.** A cultivation takes minutes; iMessage is fire-and-forget. How does an answer find its way back to a conversation that has moved on? `reply_to` + `trace_id` hint at the mechanism, but the lifecycle (acknowledge now, deliver later, handle the user messaging again mid-flight) deserves explicit treatment.
4. **Statefulness / follow-ups.** "Make it shorter" referring to the previous answer. Does the intent nutrient carry conversation context? Where does conversational state live — front door, memory layer, or trace?
