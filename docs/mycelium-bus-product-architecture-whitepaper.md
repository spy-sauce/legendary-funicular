...
4. **Statefulness / follow-ups.** *Resolved (interface) — see [statefulness contract](./mycelium-statefulness-contract.md). The capstone: §6.1/§6.3/§6.4 are now all interface-resolved.* The load-bearing move is one collapse: addressing (`reply_to/origin_channel/identity`) + lifecycle (`pending/in_flight/awaiting_confirm/done/failed`) + budget (`budget_cap/spent_so_far`) are **one durable trace record keyed by `trace_id`, one store, one lifetime** — not three stores. That record is what lets a reply survive a restart and settles the ledger. The spend-gate GO/cancel correlation binding is designed (floor); store defined by interface (SQLite = reference impl, scale question named not assumed). Cross-seam resolution with §6.1: **`awaiting_confirm` suspends the compute `deadline`** (a human's response time ≠ the compute budget) — the one place the three contracts' clocks interact. **Deferred (different lifetime, per-identity, colleague↔product boundary):** conversational memory ("make it shorter," cross-session recall), per-identity budget allowance, and the general mid-flight correlation policy.
