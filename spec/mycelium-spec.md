...
## 6a. Required Organism Artifacts

Every planted organism MUST produce, at minimum, these files at its root:

| File | Purpose |
|------|---------|
| `mycelium.yaml` | Organism config (§6) |
| `CLAUDE.md` | Project context, stack, stream tag convention |
| `NUTRIENTS.md` | Frozen shared contracts (data, design tokens, API) |
| `hyphae/HYPHA-{DOMAIN}.md` | One per agent — goal, scope, I/O, acceptance |
| `agents/{id}.ts` (or equivalent) | Agent interface stubs |
| `CELLULAR-MAP.md` | Max-concurrency execution tree (see below) |

### CELLULAR-MAP.md

The cellular map is the operator's at-a-glance view of how deep the
decomposition goes and how many parallel Claude Agent SDK sessions the
organism can fan out to at peak. It is organism-agnostic in structure —
only the agent / specialist / leaf ids are product-specific.

Required sections, in order:

1. **Heading** — `# {Organism Name} Cellular Execution Map`
2. **Gating blockquote** — restates: max-concurrency tree, gating = contract
   freeze (not fruit completion), every leaf is a Claude Agent SDK session on
   its own branch.
3. **Concurrency math** — `Depth | Count` table with Biomes (1), Specialists
   (2), Leaf specialists (3), and total concurrent sessions at peak (now vs.
   target).
4. **Gating semantics** — contrasts old wave-gating vs. new contract-freeze.
5. **Tree** — fenced code block rendering `organism → agent → specialist →
   leaf` in box-drawing characters; agents without sub-decomposition carry a
   trailing `— flat; leaf decomposition TODO` tag.
6. **What's next** — numbered list of concrete leaf-decomposition passes to
   increase peak concurrency, each with an estimated session delta.
7. **Parked for later** — deferred concerns (rate limits, circuit breakers).

The CELLULAR-MAP.md is generated by `mycelium plant` on initial scaffolding
and can be refreshed against the current repo state via `mycelium map`.
...
- [ ] A `CELLULAR-MAP.md` exists at the organism root and reflects current state
