...
const AGENT_TEMPLATE = (name) => `// Mycelium Framework — VibeSpace LLC — The network provides.
//
// Agent: ${name}
...
export function registerAgentCommand(program) {
    ...
    agent
        ...
        .option("-s, --scope <scope>", "Agent's scope/responsibility", "TODO: describe scope")
        .option("-b, --branch <branch>", "Git branch for this agent")
        ...
        if (fs.existsSync(agentFile)) {
            spinner.fail(chalk.red(`Agent ${chalk.bold(name)} already exists at ${agentFile}`));
            return;
        }
        ...
        if (fs.existsSync(configPath)) {
            ...
            const capabilities = opts.capabilities
                ? opts.capabilities.split(",").map((c) => c.trim())
                : [];
            config.agents.push({
                id: name,
                scope: opts.scope,
        ...
        spinner.succeed(chalk.greenBright(`Agent spore ${chalk.white.bold(name)} planted!`));
        console.log();
        console.log(chalk.gray("  📄 ") + chalk.yellow(agentFile));
        console.log();
        console.log(chalk.gray("  State: ") +
            ...
            chalk.gray(")"));
        console.log();
        console.log(chalk.gray.italic("  A new spore joins the network. It will grow when the time is right. 🌱"));
    });
    agent
        .command("list")
        ...
        if (agents.length === 0) {
            console.log(chalk.yellow("  🌑 No agents in the organism yet. Create one with ") +
                chalk.cyan("mycelium agent create <name>"));
            return;
        }
        console.log(chalk.magentaBright(`  🧬 ${agents.length} agent(s) in the organism:\n`));
        for (const agent of agents) {
            const blockedTag = agent.blocked_by && agent.blocked_by.length > 0
                ? chalk.red(` [blocked by: ${agent.blocked_by.join(", ")}]`)
                : chalk.green(" [unblocked]");
            console.log(chalk.white.bold(`    ${agent.id}`) +
