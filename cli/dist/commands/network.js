...
export function registerNetworkCommand(program) {
    ...
    network
        ...
        for (const agent of agents) {
            ...
            if (state === "FRUIT_READY")
                readyCount++;
            if (state === "HYPHAL_GROWTH" || state === "GERMINATING")
                activeCount++;
            table.push([
        ...
        console.log(chalk.magentaBright("  🫀 Organism Health: ") + healthBar + chalk.gray(` ${healthPct}%`));
        console.log(chalk.gray(`     ${readyCount}/${total} agents harvested, ${activeCount} actively growing`));
        console.log();
    });
    network
        .command("visualize")
        ...
        console.log(chalk.magentaBright("  🕸️  Network Topology — ") +
            chalk.white.bold(config.organism.name));
        console.log();
                ...
                for (const agent of layer) {
                    for (const nextAgent of nextLayer) {
                        if (nextAgent.blocked_by.includes(agent.id)) {
                            connections.push(chalk.gray(`           ${agent.id}`) +
                                chalk.magenta(" ──▶ ") +
        ...
        console.log();
        console.log(chalk.gray.italic("  Nutrients flow downward through the network. 🌊"));
        console.log();
    });
}
...
function generateHealthBar(pct) {
    const width = 20;
    const filled = Math.round((pct / 100) * width);
    const empty = width - filled;
    const filledStr = pct >= 80
