...
export function registerPlantCommand(program) {
    program
        .command("plant <brief>")
        .description("🌰 Plant an organism from a business brief — the SDK decomposes it into HYPHAE")
        .option("-s, --stack <name>", `Stack preset (${Object.keys(STACKS).join(", ")})`)
        ...
        const organismName = opts.name ?? path.basename(briefAbs, path.extname(briefAbs));
        const stack = getStack(opts.stack);
        if (opts.stack && !stack) {
            console.log(chalk.red(`  ❌ Unknown stack "${opts.stack}". Available: `) +
                chalk.cyan(Object.keys(STACKS).join(", ")));
            process.exit(1);
        }
        console.log();
        console.log(chalk.magentaBright.bold("  🌰 Planting organism ") +
            chalk.white.bold(organismName));
        console.log(chalk.gray("  Brief:  ") + chalk.yellow(briefAbs));
        console.log(chalk.gray("  Target: ") + chalk.yellow(targetDir));
            ...
            for await (const msg of result) {
                if (msg.type === "assistant") {
                    ...
                    for (const b of blocks) {
                        if (b.type === "tool_use") {
                            spinner.text = chalk.cyan(`Sub-agent: ${chalk.white(b.name)} ${chalk.gray(summarizeToolInput(b.name, b.input))}`);
                        }
