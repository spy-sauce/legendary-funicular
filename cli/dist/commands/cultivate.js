...
export function registerCultivateCommand(program) {
    program
        ...
        const frozenCount = contracts.filter((c) => typeof c === "object" && c.frozen).length;
        spinnerContracts.succeed(chalk.green(`${contracts.length} contract(s) validated` +
        ...
        spinnerDeps.succeed(chalk.green(`Dependency graph resolved — ${layers.length} growth phase(s) identified`));
        // Phase 4: Initializing agents
        console.log();
        console.log(chalk.magentaBright("  🧬 Initializing agents...\n"));
        for (let i = 0; i < layers.length; i++) {
            const layer = layers[i];
            console.log(chalk.gray(`  ── Phase ${i + 1} `) +
                chalk.gray("─".repeat(40)));
            for (const agent of layer) {
                const spinner = ora({
                    text: chalk.cyan(`Germinating ${chalk.white.bold(agent.id)}` +
                        chalk.gray(` (${agent.scope})`)),
                    spinner: "dots",
                    indent: 4,
                }).start();
                await sleep(250 + Math.random() * 200);
                const blockedStr = agent.blocked_by.length > 0
                    ? chalk.gray(` [after: ${agent.blocked_by.join(", ")}]`)
                    : "";
                spinner.succeed(chalk.green(`${agent.id} `) +
                    chalk.gray("→ ") +
        ...
        if (contracts.length > 0) {
            ...
            await sleep(400);
            spinnerDistribute.succeed(chalk.green(`Contracts distributed to ${agents.length} agent(s)`));
            console.log();
        }
        ...
        await sleep(400);
        spinnerHealth.succeed(chalk.green(`Health pulse active — interval: ${organism.health_pulse_interval || 30}s`));
        ...
        if (timeline && timeline.parallel_phases) {
            ...
            for (const phase of timeline.parallel_phases) {
                const activeStr = phase.active
                    .map((id) => chalk.cyan(id))
                    .join(chalk.gray(", "));
                console.log(chalk.yellow(`    ⏱  ${phase.hours}h`) +
                    chalk.gray(" — ") +
                    chalk.white(phase.description));
                console.log(chalk.gray("       Active: ") + activeStr);
            }
        }
        // ── Final output ──────────────────────────────────────
        console.log();
        ...
        console.log(chalk.gray("  Harvest threshold: ") +
            chalk.cyan(`${Math.round((organism.harvest_threshold || 0.8) * 100)}%`));
        console.log();
...
function buildLayers(agents) {
    const placed = new Set();
    const layers = [];
    let remaining = [...agents];
    while (remaining.length > 0) {
        const layer = remaining.filter((a) => (a.blocked_by || []).every((dep) => placed.has(dep)));
        if (layer.length === 0) {
