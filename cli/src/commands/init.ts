...
export function registerInitCommand(program: Command): void {
  program
    ...
    .action(async (opts) => {
      ...
      fs.writeFileSync(
        path.join(targetDir, "mycelium.yaml"),
        yamlContent,
        "utf-8"
      );
      ...
      fs.writeFileSync(
        path.join(targetDir, "contracts", ".gitkeep"),
        "",
        "utf-8"
      );
      ...
      console.log(
        chalk.gray("    ├── ") + chalk.yellow("mycelium.yaml") + chalk.gray("  — organism configuration")
      );
      console.log(
        chalk.gray("    ├── ") + chalk.yellow("contracts/") + chalk.gray("     — shared type contracts")
      );
      console.log(
        chalk.gray("    └── ") + chalk.yellow("agents/") + chalk.gray("        — agent definitions")
      );
      console.log();
      console.log(
      ...
      console.log(
        chalk.gray("    1. ") + chalk.white("Define contracts in ") + chalk.yellow("contracts/")
      );
      console.log(
        chalk.gray("    2. ") + chalk.white("Create agents with ") + chalk.cyan("mycelium agent create <name>")
      );
      console.log(
        chalk.gray("    3. ") + chalk.white("Start the organism with ") + chalk.cyan("mycelium cultivate")
      );
      console.log();
      console.log(
        chalk.gray.italic("  The mycelium awaits. The network will provide. 🌿")
      );
    });
}
