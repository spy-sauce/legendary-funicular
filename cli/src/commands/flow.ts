...
export function registerFlowCommand(program: Command): void {
  program
    .command("flow")
    .description(
      "🌊 Trigger the nutrient flow algorithm — distribute work to ready agents"
    )
    .action(async () => {
      ...
      console.log(
        chalk.magentaBright("  🌊 Nutrient Flow Algorithm") +
      ...
      const blockedAgents: Agent[] = [];
      const growingAgents: Agent[] = [];
      ...
      if (blockedAgents.length > 0) {
        ...
        for (const a of blockedAgents) {
          const waitingOn = (a.blocked_by || []).filter(
            (b) => !completedIds.has(b)
          );
          console.log(
            chalk.gray("     ├── ") +
              chalk.white.bold(a.id) +
      ...
      console.log(
        chalk.magentaBright("  📊 Flow Summary: ") +
      ...
      console.log(
        chalk.gray.italic(
          "  Nutrients flow to where they are needed. The network provides. 🌿"
        )
      );
    });
}
