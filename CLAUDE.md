...
5. **Don't write tests *for this framework repo*** — with two standing carve-outs: the security suites (`cli/src/security/*.test.ts`) and `cli/src/stacks/index.test.ts` are legit, vitest-run (`cd cli && npm test`), and gated in CI. Outside those, `--dry-run` of cultivate + `tsc --noEmit` remains the discipline; don't add new framework test surface without coordination. *Tests inside cultivated apps* (e2e, contract, smoke, regression) are first-class outputs that leaves should generate when the cultivation calls for them; this rule does not restrict that. See `docs/contract-tests-from-nutrients.md` and `docs/mycelium-eval-spec.md` for the framework-side test-generation contracts.
...
5. **Do not write tests *for the framework repo itself*** unless your HYPHA explicitly asks for them. Existing carve-outs (security suites in `cli/src/security/*.test.ts`, `cli/src/stacks/index.test.ts`) are maintained, vitest-run, and CI-gated — extend them only if your HYPHA covers that scope; otherwise `--dry-run` + `tsc --noEmit` is the discipline. (Tests *inside cultivated apps* — e2e, contract, smoke, regression — are first-class deliverables when a HYPHA produces an app. This rule does not restrict that.)
